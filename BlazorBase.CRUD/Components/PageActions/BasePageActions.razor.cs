...
public partial class BasePageActions
{
    ...
    [Parameter] public BaseExternalPageActionFragement? ExternalPageActionFragement { get; set; } = null;
    ...
    private async Task InvokePageAction(PageAction action)
    {
        if (action.RenderComponentByActionArgs != null)
            await SetActionRenderFragementAsync(RenderComponentByActionArgs(action.RenderComponentByActionArgs));
...
#pragma warning restore ASP0006 // Do not use non-literal sequence numbers
    ...
    protected async Task OnComponentCanBeRemovedAsync(RenderComponentByActionArgs args, object? optionalParameter)
    {
        ...
        await SetActionRenderFragementAsync(null);
        await OnPageActionInvoked.InvokeAsync(exception);
    }

    protected Task SetActionRenderFragementAsync(RenderFragment? renderFragment)
    {
        if (ExternalPageActionFragement == null)
            CurrentActionRenderFragment = renderFragment;
        else
        {
            CurrentActionRenderFragment = null;
            return ExternalPageActionFragement.SetPageActionFragementAsync(renderFragment);
        }
