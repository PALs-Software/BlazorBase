...
window.blazorise = {
    ...
    cleanupClosableComponents: () => {
        window.blazorise.closableComponents = window.blazorise.closableComponents.filter(component => {
            return document.getElementById(component.elementId) !== null;
        });
    },

    getLastClosableComponent: () => {
        window.blazorise.cleanupClosableComponents();
        if (window.blazorise.closableComponents.length > 0)
            return window.blazorise.closableComponents[window.blazorise.closableComponents.length - 1];
...
document.addEventListener('mouseup', function handler(evt) {
    if (evt.target === window.blazorise.lastClickedDocumentElement && window.blazorise.closableComponents && window.blazorise.closableComponents.length > 0) {
        const lastClosable = window.blazorise.getLastClosableComponent();
